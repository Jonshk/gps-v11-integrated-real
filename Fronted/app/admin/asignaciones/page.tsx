...
export default function AsignacionesPage() {
  ...
  const S = {
    ...
    select: { width:"100%", padding:"8px 12px", background:t.card, border:`1px solid ${t.border}`, borderRadius:8, color:t.text, fontSize:12, outline:"none" },
  };
  ...
  return (
    <div style={S.page}>
      <div style={{ marginBottom:28 }}>
        ...
        <h1 style={{ fontSize:26, fontWeight:800, letterSpacing:"-0.8px", margin:"0 0 4px", color:t.text }}>Asignaciones</h1>
        <p style={{ color:t.textFaint, fontSize:13, margin:0 }}>Vincula cada cliente con su vehículo y dispositivo GPS</p>
      </div>
      ...
      <div style={{ display:"grid", gridTemplateColumns:"repeat(3,1fr)", gap:12, marginBottom:24 }}>
        {[
          ...
          <div key={s.n} style={{ ...S.glass, padding:"16px 18px" }}>
            ...
            <div style={{ color:t.textFaint, fontSize:11, lineHeight:1.5 }}>{s.sub}</div>
          </div>
        ))}
      </div>

      {msg && <div style={{ marginBottom:14, padding:"9px 14px", background:"rgba(34,197,94,0.08)", border:"1px solid rgba(34,197,94,0.2)", borderRadius:8, color:"#16a34a", fontSize:12, fontWeight:600 }}>{msg}</div>}
      ...
      {loading ? (
        ...
        <div style={{ ...S.glass, padding:"60px 0", textAlign:"center", color:t.textFaint, fontSize:14 }}>Sin clientes. Crea uno primero.</div>
      ) : (
        <div style={{ ...S.glass, overflow:"hidden" }}>
          <div style={{ display:"grid", gridTemplateColumns:"1.5fr 2fr 2fr 140px", padding:"12px 20px", background:t.input, borderBottom:`1px solid ${t.border}` }}>
          ...
          {clients.map((c,idx)=>{
            ...
            return (
              <div key={c.id} style={{ display:"grid", gridTemplateColumns:"1.5fr 2fr 2fr 140px", padding:"16px 20px", alignItems:"center", borderBottom:idx<clients.length-1?`1px solid ${t.border}`:"none", transition:"background 0.15s" }}
                ...
                <div>
                  ...
                  <div style={{ color:t.textFaint, fontSize:11, fontFamily:"monospace", marginTop:3 }}>@{c.username}</div>
                </div>
                ...
                <div>
                  {ready ? (
                    <span style={{ display:"inline-flex", alignItems:"center", gap:5, padding:"5px 12px", borderRadius:999, background:"rgba(34,197,94,0.1)", border:"1px solid rgba(34,197,94,0.25)", color:"#16a34a", fontSize:11, fontWeight:700 }}>
                      <span style={{ width:5, height:5, borderRadius:"50%", background:"#22c55e" }}/> Listo
                    </span>
                  ) : (
                    <span style={{ display:"inline-flex", alignItems:"center", gap:5, padding:"5px 12px", borderRadius:999, background:"rgba(245,158,11,0.1)", border:"1px solid rgba(245,158,11,0.25)", color:"#d97706", fontSize:11, fontWeight:700 }}>
                      ⚠ Incompleto
                    </span>
                  )}
                </div>
