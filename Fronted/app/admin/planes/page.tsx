...
export default function PlanesPage() {
  ...
  const glass = { background:t.card, backdropFilter:"blur(20px)", border:`1px solid ${t.border}`, borderRadius:18, boxShadow:"0 2px 20px rgba(0,0,0,0.08)" };
  const S = {
    ...
    btnGhost: { padding:"8px 16px", background:"transparent", color:t.textMuted, border:`1px solid ${t.border}`, borderRadius:8, fontSize:12, cursor:"pointer" } as React.CSSProperties,
    field:    { marginBottom:16 } as React.CSSProperties,
    modal:    { background:t.sidebar, border:`1px solid ${t.border}`, borderRadius:20, padding:32, width:540, maxHeight:"90vh", overflowY:"auto" as const, boxShadow:"0 24px 60px rgba(0,0,0,0.4)" },
  };
    ...
    <div style={S.page}>
      <div style={{ display:"flex", alignItems:"flex-start", justifyContent:"space-between", marginBottom:28 }}>
        <div>
          ...
          <h1 style={{ fontSize:26, fontWeight:800, letterSpacing:"-0.8px", margin:"0 0 4px", color:t.text }}>Planes y precios</h1>
          <p style={{ color:t.textFaint, fontSize:13, margin:0 }}>Los cambios se publican automáticamente en gpscontrolec.com</p>
        </div>
        <button style={S.btnGhost} onClick={handleReset}>Restaurar defaults</button>
      </div>

      {msg && (
        <div style={{ marginBottom:20, padding:"10px 16px", background:msg.ok?"rgba(34,197,94,0.08)":"rgba(232,35,42,0.08)", border:`1px solid ${msg.ok?"rgba(34,197,94,0.2)":"rgba(232,35,42,0.2)"}`, borderRadius:10, color:msg.ok?"#16a34a":"#e8232a", fontSize:13, fontWeight:600 }}>
          {msg.text}
        </div>
      )}
      ...
      {loading ? (
        <div style={{ display:"grid", gridTemplateColumns:"repeat(3,1fr)", gap:16 }}>
          {[1,2,3].map(i=><div key={i} style={{ height:320, borderRadius:18, background:t.card, animation:"shimmer 1.5s infinite" }}/>)}
        </div>
      ) : (
        <div style={{ display:"grid", gridTemplateColumns:"repeat(3,1fr)", gap:16 }}>
          {plans.map(plan=>(
            <div key={plan.id} style={{ ...glass, padding:28, position:"relative", opacity:plan.active?1:0.6, transition:"transform 0.2s, box-shadow 0.2s",
              border:plan.featured?`1px solid rgba(232,35,42,0.3)`:`1px solid ${t.border}` }}
              onMouseEnter={e=>{ e.currentTarget.style.transform="translateY(-3px)"; e.currentTarget.style.boxShadow=plan.featured?"0 16px 40px rgba(232,35,42,0.15)":"0 16px 40px rgba(0,0,0,0.15)"; }}
              onMouseLeave={e=>{ e.currentTarget.style.transform="translateY(0)"; e.currentTarget.style.boxShadow="0 2px 20px rgba(0,0,0,0.08)"; }}>
              ...
              {!plan.active && <div style={{ position:"absolute", top:14, right:14, background:t.border, border:`1px solid ${t.border}`, color:t.textFaint, fontSize:9, fontWeight:700, padding:"2px 8px", borderRadius:999, textTransform:"uppercase" as const }}>Inactivo</div>}

              <div style={{ fontSize:17, fontWeight:800, color:t.text, marginBottom:4 }}>{plan.name}</div>
              <div style={{ color:t.textFaint, fontSize:13, marginBottom:20 }}>{plan.desc}</div>
              ...
              <div style={{ marginBottom:22, paddingBottom:22, borderBottom:`1px solid ${t.border}` }}>
                ...
                <span style={{ color:t.textMuted, fontSize:13, marginLeft:6 }}>{plan.sub}</span>
              </div>

              <ul style={{ listStyle:"none", padding:0, margin:"0 0 22px", display:"flex", flexDirection:"column", gap:10 }}>
                {plan.features.map((f,i)=>(
                  <li key={i} style={{ display:"flex", alignItems:"flex-start", gap:10, fontSize:13, color:t.textMuted, lineHeight:1.4 }}>
                    <div style={{ width:16, height:16, borderRadius:"50%", background:"rgba(232,35,42,0.1)", border:"1px solid rgba(232,35,42,0.2)", display:"flex", alignItems:"center", justifyContent:"center", flexShrink:0, marginTop:1 }}>
      ...
      {editing && (
        <div style={{ position:"fixed", inset:0, background:"rgba(0,0,0,0.5)", backdropFilter:"blur(4px)", zIndex:100, display:"flex", alignItems:"center", justifyContent:"center", padding:20 }} onClick={()=>setEditing(null)}>
          <div style={S.modal} onClick={e=>e.stopPropagation()}>
            <h2 style={{ color:t.text, fontSize:18, fontWeight:800, marginBottom:6 }}>Editar — {editing.name}</h2>
            <p style={{ color:t.textFaint, fontSize:12, marginBottom:24 }}>Los cambios se publican automáticamente en la web pública.</p>
            ...
            <div style={{ display:"flex", gap:24, marginBottom:20 }}>
              <label style={{ display:"flex", alignItems:"center", gap:8, cursor:"pointer", fontSize:13, color:t.text }}><input type="checkbox" checked={editing.featured} onChange={e=>sf("featured",e.target.checked)}/> Más popular</label>
              <label style={{ display:"flex", alignItems:"center", gap:8, cursor:"pointer", fontSize:13, color:t.text }}><input type="checkbox" checked={editing.active} onChange={e=>sf("active",e.target.checked)}/> Activo en web</label>
            </div>
