    ...
    <div style={S.page}>
      <div style={{ display:"flex", alignItems:"flex-start", justifyContent:"space-between", marginBottom:28 }}>
        <div>
          ...
          <h1 style={{ fontSize:26, fontWeight:800, letterSpacing:"-0.8px", margin:"0 0 4px", color:"#1a1a2e" }}>Vehículos</h1>
          <p style={{ color:"#9ca3af", fontSize:13, margin:0 }}>{vehicles.length} vehículos registrados</p>
        </div>
      ...
      {loading ? (
        ...
        <div style={{ ...glass, padding:"60px 0", textAlign:"center", color:"#9ca3af", fontSize:14 }}>Sin vehículos. Crea el primero.</div>
      ) : (
        <div style={{ ...glass, overflow:"hidden" }}>
          <div style={{ display:"grid", gridTemplateColumns:"2fr 120px 1.5fr 1fr 120px", padding:"12px 20px", background:"rgba(0,0,0,0.02)", borderBottom:"1px solid rgba(0,0,0,0.06)" }}>
          ...
          {vehicles.map((v, idx) => {
            ...
            return (
              <div key={v.id} style={{ display:"grid", gridTemplateColumns:"2fr 120px 1.5fr 1fr 120px", padding:"16px 20px", alignItems:"center", borderBottom:idx<vehicles.length-1?"1px solid rgba(0,0,0,0.05)":"none", transition:"background 0.15s" }}
                onMouseEnter={e=>(e.currentTarget.style.background="rgba(0,0,0,0.02)")}
                onMouseLeave={e=>(e.currentTarget.style.background="transparent")}>
                <div>
                  <div style={{ fontWeight:700, fontSize:14, color:"#1a1a2e" }}>{v.name}</div>
                  <div style={{ color:"#9ca3af", fontSize:10, fontFamily:"monospace", marginTop:3 }}>{v.id}</div>
                </div>
                <div>
                  <span style={{ display:"inline-flex", alignItems:"center", gap:5, padding:"4px 10px", borderRadius:999, fontSize:11, fontWeight:700, background:`${st.color}0f`, border:`1px solid ${st.color}25`, color:st.color }}>
                    <span style={{ width:5, height:5, borderRadius:"50%", background:st.dot }}/>{st.label}
                  </span>
                </div>
                <div style={{ fontFamily:"monospace", fontSize:12, color:"#6b7280" }}>{v.lat.toFixed(5)}, {v.lng.toFixed(5)}</div>
                <div style={{ fontSize:13, color:v.geofence?"#374151":"#d1d5db" }}>{v.geofence||"—"}</div>
      ...
      {modal && (
        <div style={{ position:"fixed", inset:0, background:"rgba(0,0,0,0.4)", backdropFilter:"blur(4px)", zIndex:100, display:"flex", alignItems:"center", justifyContent:"center", padding:20 }} onClick={close}>
          <div style={{ background:"#fff", border:"1px solid rgba(0,0,0,0.08)", borderRadius:20, padding:32, width:440, boxShadow:"0 24px 60px rgba(0,0,0,0.15)" }} onClick={e=>e.stopPropagation()}>
            <h2 style={{ color:"#1a1a2e", fontSize:18, fontWeight:800, marginBottom:6 }}>{modal==="create"?"Nuevo vehículo":"Editar vehículo"}</h2>
            <p style={{ color:"#9ca3af", fontSize:12, marginBottom:24 }}>Las coordenadas se actualizan solas cuando el GPS reporta posición.</p>
