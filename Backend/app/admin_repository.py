...
def get_device(device_id: str):
    with get_conn() as conn:
        ...
        cur.execute(
            "SELECT * FROM gps_devices WHERE id=%s",
            (device_id,)
        )
...
def update_device(device_id: str, data: dict):
    ...
    with get_conn() as conn:
        ...
        """, (
            merged.get("name"),
            merged.get("sim_number"),
            merged.get("model"),
            merged.get("imei"),
            merged.get("vehicle_id") or None,
            to_bool(merged.get("active")),
            merged.get("notes"),
            device_id,
        ))
...
def delete_device(device_id: str):
    with get_conn() as conn:
        ...
        cur.execute(
            "DELETE FROM gps_devices WHERE id=%s",
            (device_id,)
        )
...
def get_client(client_id: str):
    with get_conn() as conn:
        ...
        cur.execute(
            "SELECT * FROM app_clients WHERE id=%s",
            (client_id,)
        )
...
def delete_client(client_id: str):
    with get_conn() as conn:
        ...
        cur.execute(
            "DELETE FROM app_clients WHERE id=%s",
            (client_id,)
        )
...
def toggle_client_active(client_id: str):
    cli = get_client(client_id)

    if not cli:
        return None

    new_value = not to_bool(cli.get("active"))
...
def get_client_for_login(username: str, password: str):
    with get_conn() as conn:
        cur = conn.cursor()
